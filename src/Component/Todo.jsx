...
export default class Todo extends React.Component {
	...
	handleClick = (event) => {
		...
		this.setState({
			list: [...this.state.list, this.state.text], text: '',
		});
	}
	onDel = (index) => {
		this.setState({
	...
	render() {
		let lists = this.state.list.map((item, index) => React.createElement('li', {}, item, React.createElement('button', {onClick: () => this.onDel(index)}, 'X')));
